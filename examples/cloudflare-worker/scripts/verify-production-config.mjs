...
const vars = asRecord(config.vars, 'vars');
const assets = asRecord(config.assets, 'assets');
const secrets = asRecord(config.secrets, 'secrets');
...
if (!chatCoreBaseUrl) {
  ...
  requireHttpsOrigin(chatCoreBaseUrl, 'vars.CHAT_CORE_BASE_URL');
...
if (!Array.isArray(secrets.required) || !secrets.required.includes('CHAT_CORE_SERVICE_TOKEN')) {
  errors.push('secrets.required must include CHAT_CORE_SERVICE_TOKEN.');
}

if (!hasProductionRoute(config) && process.env.ALLOW_WORKER_DEV_DEPLOY !== 'true') {
  errors.push(
