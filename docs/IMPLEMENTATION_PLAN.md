...
launcher's click-to-open-chat behavior unchanged. A transparent hover bridge preserves the same
...
pattern with a matching bottom rule and a decorative 2.8-second pulsing live dot beside the title.
The single-line header omits the response-time subtitle and gives Clear and Close the same compact
control height. Text and panel controls share the panel corner radius;
